import { FabricObject, Group, TPointerEvent, TPointerEventInfo, Rect, CanvasEvents, ActiveSelection } from "fabric";
...
export class MouseAction extends Plugin {
  ...
  public handleMouseUp = (e: TPointerEventInfo<TPointerEvent>): void => {
    this.state.selected
      .filter(obj => obj.type !== 'group')
    ...
    if (this.state.isMouseDown && this.state.selected.length && this.state.targetGroup) {
      ...
      console.log(activeSelection instanceof ActiveSelection)
      this.addToFrame();
    }
  ...
  private addToFrame(): void {
    ...
    if (!group) return;
    Promise.all(this.state.selected.filter(obj => !group.contains(obj)).map(obj => obj.clone()))
      .then(clonedObjects => {
        this._render._FC.remove(...this.state.selected);
        group.add(...clonedObjects);
        group.setCoords();
